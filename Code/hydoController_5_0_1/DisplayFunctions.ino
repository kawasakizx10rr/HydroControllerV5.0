...
void OuterMenuIcons() {
  ...
  drawPageIcon(1, 17, 187, graphIcon, 372, 66, 45);
  ...
  drawPageIcon(9, 437, 17, doserIcon, 467, 55, 68);
  drawPageIcon(10, 527, 15, lightIcon, 344, 41, 67);
...
void drawPageIcon(const uint8_t& a_page, const short& a_x, const short a_y, const uint8_t* a_icon, const int& a_arraySize, const uint8_t& a_width, const uint8_t& a_height) {
  if (display::page == a_page) {
    tft.drawXbmpArray(a_icon, a_arraySize, a_x, a_y, a_width, a_height, RA8875_BLUE, 1);
  }
  else if (display::previousPage == a_page || display::refreshPage) {
    tft.drawXbmpArray(a_icon, a_arraySize, a_x, a_y, a_width, a_height, display::RA8875_DARKGREY, 1);
  }
}

void drawPageIconQuick(const uint8_t& a_page, const short& a_x, const short a_y, const uint8_t* a_icon, const int& a_arraySize, const uint8_t& a_width, const uint8_t& a_height) {
  if (display::page == a_page) {
    tft.drawXbmpArray(a_icon, a_arraySize, a_x, a_y, a_width, a_height, RA8875_BLUE, 1);
  }
  else {
...
void drawGraphIcons() {
  tft.drawXbmpArray(miniPmIcon, 115, 747, 98, 40, 23, display::showTdsGraph ? display::RA8875_PURPLE : display::RA8875_DARKGREY, 1);
  tft.drawXbmpArray(miniPhIcon, 112, 747, 131, 39, 23, display::showPhGraph ? display::RA8875_LIGHT_GREEN : display::RA8875_DARKGREY, 1);
  tft.drawXbmpArray(miniEcIcon, 109, 748, 166, 38, 23, display::showEcGraph ? RA8875_YELLOW : display::RA8875_DARKGREY, 1);
  tft.drawXbmpArray(miniCo2Icon, 130, 746, 199, 40, 26, display::showCo2Graph ? RA8875_MAGENTA : display::RA8875_DARKGREY, 1);
  tft.drawXbmpArray(miniWaterTempIcon, 108, 753, 234, 27, 32, display::showWaterTempGraph ? display::RA8875_ORANGE : display::RA8875_DARKGREY, 1);
  tft.drawXbmpArray(miniWaterLevelIcon, 101, 754, 273, 27, 30, display::showWaterLvlGraph ? RA8875_BLUE : display::RA8875_DARKGREY, 1);
  tft.drawXbmpArray(miniFanOneIcon, 157, 749, 312, 35, 36, display::showFanOneGraph ? RA8875_WHITE : display::RA8875_DARKGREY, 1);
  tft.drawXbmpArray(miniFanTwoIcon, 157, 749, 356, 35, 36, display::showFanTwoGraph ? RA8875_BLACK : display::RA8875_DARKGREY, 1);
  tft.drawXbmpArray(miniAirTempIcon, 121, 754, 396, 27, 36, display::showAirTempGraph ? RA8875_RED : display::RA8875_DARKGREY, 1);
  tft.drawXbmpArray(miniHumidityIcon, 136, 750, 438, 34, 32, display::showHumidityGraph ? display::RA8875_SEABLUE : display::RA8875_DARKGREY, 1);
...
void drawEcConversionButton(const int& a_x, const int& a_y) {
  ...
  tft.setFontScale(1);
  tft.setTextColor(RA8875_BLACK);
  tft.fillRoundRect(a_x, a_y, 60, 38, 5, display::RA8875_DARKGREY);
...
void drawSensorSlide(const float & a_sensorData, const float & a_minTarget, const float & a_maxTarget, float * a_arrayData, const uint8_t & a_percision) {
  if (display::refreshPage) {
...
void displaySetRTCTime() {
  ...
  if (display::refreshPage || previousUserRTCdayOfWeek != user::rtcDayOfWeek) {
    ...
    if (display::setRtcTimePos == 0) {
      int dayWidth = tft.getStringWidth(daysOfWeek[user::rtcDayOfWeek]);
      tft.fillRect(300, 180, dayWidth, 45, RA8875_WHITE);
    }
  ...
  if (display::refreshPage || previousUserRTCDay != user::rtcDay || previousUserRTCMonth != user::rtcMonth || previousUserRTCYear != user::rtcYear) {
    ...
    if (display::setRtcTimePos == 1) {
      int dateWidth = tft.getStringWidth(dateStr);
      tft.fillRect(300, 235, dateWidth, 42, RA8875_WHITE);
    }
    ...
    if (display::setRtcTimePos == 2) {
      int dateWidth = tft.getStringWidth(dateStr);
      tft.fillRect(tft.getFontX(), 235, dateWidth, 42, RA8875_WHITE);
    }
    tft.print(dateStr);
    //
    tft.print("/");
  ...
  if (display::refreshPage || previousUserRTCMinute != user::rtcMinute || previousUserRTCSecond != user::rtcSecond || previousUserRtcHour != user::rtcHour) {
    ...
    if (display::setRtcTimePos == 4) {
      int timeWidth = tft.getStringWidth(timeStr);
      tft.fillRect(299, 290, timeWidth, 42, RA8875_WHITE);
    }
    ...
    if (display::setRtcTimePos == 5) {
      int timeWidth = tft.getStringWidth(timeStr);
      tft.fillRect(tft.getFontX(), 290, timeWidth, 42, RA8875_WHITE);
    }
    tft.print(timeStr);
    //
    tft.print(":");
  ...
  int& a_previousValue, int& a_doserPosition, const uint8_t & a_symbol, const int& a_yOffSet)
{
  int startPosition = 0;
  if (display::refreshPage) {
    tft.drawXbmpArray(doserIcon, 467, a_x + 17, a_y + 6, 55, 68, RA8875_BLACK, 2);
  }
...
void drawTwoValues(
  const int& a_startX, float a_value, const unsigned int& a_color, const uint8_t & a_precison,
  const int& a_startX2, float a_vlaue2, const unsigned int& a_color2, const uint8_t & a_precison2,
...
void displaySetAfkTime() {
  ...
  if (display::refreshPage || previousInterval != user::afkTime) {
    ...
    tft.fillRect(startX, 234, endX - startX, 50, display::RA8875_LIGHT_PINK);
...
void displaySetGraphInterval() {
  ...
  if (display::refreshPage || previousInterval != user::graphInterval) {
    ...
    tft.setCursor(startX, 234);
    tft.print((uint32_t)user::graphInterval);
