...
uint16_t getSetProfile(uint16_t a_address, const device::eepromModes a_mode) {
  ...
  eepromGetSet(a_address, a_mode, user::targetDayMaxHumidity);     
  eepromGetSet(a_address, a_mode, user::targetNightMinHumidity);            
  eepromGetSet(a_address, a_mode, user::targetNightMaxHumidity);      
  eepromGetSet(a_address, a_mode, user::selectedIrPos);   
  eepromGetSet(a_address, a_mode, user::acPollTime);   
  eepromGetSet(a_address, a_mode, user::disableAcTemp);   
  eepromGetSet(a_address, a_mode, user::disableAcHum);   
  eepromGetSet(a_address, a_mode, user::acControlTemp);   
  eepromGetSet(a_address, a_mode, user::acControlHumidity);   
  eepromGetSet(a_address, a_mode, user::acSwingVertical);   
  eepromGetSet(a_address, a_mode, user::acSwingHorizontal);   
  eepromGetSet(a_address, a_mode, user::acTurboMode);      
