...
void continueMessage(const char* a_text, const float& a_num, const uint8_t a_precision, const bool& a_showContiue, const bool& a_showCancel, bool a_refresh) {
  ...
  for (uint16_t i = 0; i < strlen(a_text); i++) {
    char c = a_text[i];
    if (c == '$' && a_num != -1) {
      numX = tft.getFontX();
      numY = tft.getFontY();
...
void setupMessage(int16_t& a_x, int16_t& a_y, const char* a_message) {
  ...
  // Work out the dialog height
  for (uint16_t i = 0; i < strlen(a_message); i++) {
    char c = a_message[i];
    if (charPos < 32)
      charBuffer[charPos++] = c;
...
void drawSensorSlide(const float& a_sensorData, const float& a_minTarget, const float& a_maxTarget, float* a_arrayData, uint8_t a_percision) {
  ...
  if (display::refreshPage || device::newGraphData) 
    drawGraph(95, 300, 164, tft.width() - 95, 5, arrayData, 1, sensor::sensorArrayPos, a_percision, graphColors);
}
