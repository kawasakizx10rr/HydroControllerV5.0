...
void displayWarnings() {
  ...
  if (device::sensorsReady && millis() - prevWarnMillis >= 1000UL) {
    ...
    if (device::lightOn) {
      if (!user::convertToF)
        setErrorState(sensor::airTemp, user::targetDayMinAirTemp, user::targetDayMaxAirTemp, user::airTempErrorMargin, user::disableAirTempWarnings, device::airTempErrorState);
      else
        setErrorState(convertToF(sensor::airTemp), user::targetDayMinAirTempF, user::targetDayMaxAirTempF, user::airTempErrorMarginF, user::disableAirTempWarnings, device::airTempErrorState);
      setErrorState(sensor::humidity, user::targetDayMinHumidity, user::targetDayMaxHumidity, user::humidityErrorMargin, user::disableHumidityWarnings, device::humidityErrorState);
    }
    else {
      if (!user::convertToF)
        setErrorState(sensor::airTemp, user::targetNightMinAirTemp, user::targetNightMaxAirTemp, user::airTempErrorMargin, user::disableAirTempWarnings, device::airTempErrorState);
      else
        setErrorState(convertToF(sensor::airTemp), user::targetNightMinAirTempF, user::targetNightMaxAirTempF, user::airTempErrorMarginF, user::disableAirTempWarnings, device::airTempErrorState);
      setErrorState(sensor::humidity, user::targetNightMinHumidity, user::targetNightMaxHumidity, user::humidityErrorMargin, user::disableHumidityWarnings, device::humidityErrorState);
    }
    //
