void initializeDevice() {
  if (device::globalDebug) {
    Serial.print("SPI max speed: "); Serial.print(MAXSPISPEED / 1000000UL); Serial.println("Mhz");
  }
  ...
  tft.displayOn(true);
  if (device::globalDebug) {
    Serial.print("Hydro Master v"); Serial.print(device::versionNumber); Serial.println(" started...");
  }
  ...
  tft.fillRect(120, 370, 400, 24, RA8875_BLACK);
  tft.setTextColor(RA8875_WHITE, RA8875_BLACK);
  tft.print(120, 370, "Loading VL53L0X...");
  delay(200);
  // if (!lox.begin()) {
  //     tft.fillRect(120, 370, 400, 24, RA8875_BLACK);
  //     tft.setTextColor(RA8875_RED, RA8875_BLACK);
  //     tft.print(120, 370, "Failed to load VL53L0X");
  ...
  if (user::lightMode == 0 && restartLightingTimer()) {
    ...
    if (device::globalDebug)
      Serial.println("Auto started light on boot");
  }
  // // Load the Co2 sensor
  ...
  if (wifi::wifiEnabled) {
    ...
    if (device::globalDebug) {
      Serial.print("Started AP on IP address: "); Serial.println(WiFi.softAPIP());
      Serial.print("AP SSID: "); Serial.println(wifi::ssid);
      Serial.print("AP password: "); Serial.println(wifi::password);
    }
...
void analogWriteChnl(const uint8_t& a_channel, const uint16_t& a_dutyCycle) {
  if (device::globalDebug) {
    Serial.print("Channel "); Serial.print(a_channel); Serial.print(" set to ");
  }
  if(a_dutyCycle == 0) {
    if (device::globalDebug)
      Serial.println("OFF");
...
void setTimer(uint8_t& a_min, uint8_t& a_hour, const unsigned int& a_interalInMinutes) {
  ...
  if (device::globalDebug) {
    Serial.print("Fan will be turn back on at "); Serial.print(a_hour); Serial.print(":"); Serial.print(a_min); Serial.print(":"); Serial.println(":00");
  } 
}
...
void setRgbLed(const int a_red, const int a_green, const int a_blue) {
  if (!user::disableLED) {
    if (device::globalDebug) {
      Serial.print("RGB LED: "); Serial.print(a_red); Serial.print(","); Serial.print(a_green); Serial.print(","); Serial.println(a_blue);
    }
...
float adjustValue(float a_val, float a_increment, const float& a_min, const float& a_max) {
  ...
  if (device::globalDebug) {
    Serial.print("Adjust value = "); Serial.println(a_val, 2);
  }
  return a_val;
}
...
unsigned long adjustInterval(unsigned long a_val, const int a_increment) {
  ...
  if (device::globalDebug) {
    Serial.print("Adjust Interval = "); Serial.println(a_val);
  }
  return a_val;
}
...
void adjustrtc() {
  if (device::globalDebug)
    Serial.println("Setting RTC rtc");
...
void prime(const uint8_t& a_doser, const uint8_t& a_channel, const int& a_speed) {
  ...
  if (a_speed != 0) {
    if (a_doser != previousDoser) {
      if (device::globalDebug) {
        Serial.print("Priming doser: "); Serial.print(a_doser); Serial.print(" at PWM: "); Serial.println(a_speed);
      }
    ...
    if (millis() - primeRunTime >= 1000UL) {
      ...
      ++currentDosingAmount;
      if (device::globalDebug) {
        Serial.print("Primed "); Serial.print(currentDosingAmount); Serial.println(" mls");
      }
      beep();
    }
  }
    ...
    if (device::globalDebug) {
      Serial.print("Stopping doser "); Serial.println(a_doser);
    }
...
void saveLogMessage(const uint8_t& a_logType) {
  ...
  if (message::systemLogPos == message::maxLogs) {
    if (device::globalDebug)
      Serial.println("Pushing logs back...");
  ...
  if (device::globalDebug) {
    Serial.print("Added new log type "); Serial.print(a_logType); Serial.print(" at position "); Serial.print(arraypos); Serial.print(", LOG: "); Serial.print(logPretext); Serial.println(message::timeStrArray[arraypos]);
  }
  if (message::systemLogPos < message::maxLogs) 
...
void adjustFanMode(const float a_sensor, device::controlOptions& a_fanOption, const sensor::airStates a_state, const char* a_str, const float a_percent, const float a_minTarget, const float a_maxTarget) {
  if (greaterThan(a_sensor, a_maxTarget, 2) && a_state == sensor::IS_FALLING) {
    if (device::globalDebug) {
      Serial.print("The "); Serial.print(a_str); Serial.print(" "); Serial.print(a_sensor, 2); Serial.print(" is > max target "); Serial.print(a_maxTarget, 2); Serial.print(", by "); Serial.print(a_percent); Serial.print("% but the "); Serial.print(a_str); Serial.println(" is falling.");
    }
    a_fanOption = device::SPEED_IDLE;
  }
  else if (greaterThan(a_sensor, a_maxTarget, 2) && a_state == sensor::IS_RISING) {
    if (device::globalDebug) {
      Serial.print("The "); Serial.print(a_str); Serial.print(" "); Serial.print(a_sensor, 2); Serial.print(" is > max target "); Serial.print(a_maxTarget, 2); Serial.print(", by "); Serial.print(a_percent); Serial.print("% but the "); Serial.print(a_str); Serial.println(" is rising.");
    }
    a_fanOption = device::SPEED_UP; //device::SPEED_MAX;
  }
  else if (greaterThan(a_sensor, a_maxTarget, 2) && a_state == sensor::IS_SAME) {
    if (device::globalDebug) {
      Serial.print("The "); Serial.print(a_str); Serial.print(" "); Serial.print(a_sensor, 2); Serial.print(" is > max target "); Serial.print(a_maxTarget, 2); Serial.print(", by "); Serial.print(a_percent); Serial.print("% yet the "); Serial.print(a_str); Serial.println(" remains the same.");
    }
    a_fanOption = device::SPEED_UP;
  }
  else if (lessThan(a_sensor, a_minTarget, 2) && a_state == sensor::IS_FALLING) {
    if (device::globalDebug) {
      Serial.print("The "); Serial.print(a_str); Serial.print(" "); Serial.print(a_sensor, 2); Serial.print(" is < min target "); Serial.print(a_minTarget, 2); Serial.print(", by "); Serial.print(a_percent); Serial.print("% but the "); Serial.print(a_str); Serial.println(" is falling.");
    }
    a_fanOption = device::SPEED_DOWN; //device::SPEED_MIN;
  }
  else if (lessThan(a_sensor, a_minTarget, 2) && a_state == sensor::IS_RISING) {
    if (device::globalDebug) {
      Serial.print("The "); Serial.print(a_str); Serial.print(" "); Serial.print(a_sensor, 2); Serial.print(" is < min target "); Serial.print(a_minTarget, 2); Serial.print(", by "); Serial.print(a_percent); Serial.print("% but the "); Serial.print(a_str); Serial.println(" is rising.");
    }
    a_fanOption = device::SPEED_IDLE;
  }
  else if (lessThan(a_sensor, a_minTarget, 2) && a_state == sensor::IS_SAME) {
    if (device::globalDebug) {
      Serial.print("The "); Serial.print(a_str); Serial.print(" "); Serial.print(a_sensor, 2); Serial.print(" is < min target "); Serial.print(a_minTarget, 2); Serial.print(", by "); Serial.print(a_percent); Serial.print("% yet the "); Serial.print(a_str); Serial.println(" remains the same.");
    }
    a_fanOption = device::SPEED_DOWN;
  }
  else if (a_state == sensor::IS_FALLING) {
    if (device::globalDebug) {
      Serial.print("The "); Serial.print(a_str); Serial.print(" "); Serial.print(a_sensor, 2); Serial.print(" is with min "); Serial.print(a_minTarget, 2); Serial.print(" max "); Serial.print(a_maxTarget, 2); Serial.print(" targets, but the "); Serial.print(a_str); Serial.println(" is falling.");
    }
    ...
    if (device::globalDebug) {
      Serial.print("The "); Serial.print(a_str); Serial.print(" "); Serial.print(a_sensor, 2); Serial.print(" is with min "); Serial.print(a_minTarget, 2); Serial.print(" max "); Serial.print(a_maxTarget, 2); Serial.print(" targets, but the "); Serial.print(a_str); Serial.println(" is rising.");
    }
    if (!(user::fansControlTemperature && user::fansControlHumidity))
    ...
    if (device::globalDebug) {
      Serial.print("The "); Serial.print(a_str); Serial.print(" "); Serial.print(a_sensor, 2); Serial.print(" is with min "); Serial.print(a_minTarget, 2); Serial.print(" max "); Serial.print(a_maxTarget, 2); Serial.print(" targets, and the "); Serial.print(a_str); Serial.println(" remains the same.");
    }
    if (!(user::fansControlTemperature && user::fansControlHumidity))
      a_fanOption = device::SPEED_IDLE;
  }
  if (device::globalDebug) {    
    Serial.print("The fan mode for "); Serial.print(a_str); Serial.print(" is set to ");
    if (a_fanOption == device::SPEED_IDLE)
      ...
      Serial.println("speed down slowly");
    else if (a_fanOption == device::SPEED_UP_SLOW)
      Serial.println("speed up slowly");
  }
}

bool i2cScanner(const uint8_t a_address) {
  if (device::globalDebug) {
    Serial.print("Scanning for I2C address 0x"); Serial.println(a_address, HEX);
  }
  Wire.begin();
  Wire.beginTransmission(a_address);
  if (Wire.endTransmission () == 0) { // Receive 0 = success (ACK response) 
    if (device::globalDebug) {
      Serial.print("Found address: 0x"); Serial.println(a_address, HEX);
    }
    return true;
  }
  if (device::globalDebug) {
    Serial.print("Failed to find address: 0x"); Serial.println(a_address, HEX);
  }
  return false;
}