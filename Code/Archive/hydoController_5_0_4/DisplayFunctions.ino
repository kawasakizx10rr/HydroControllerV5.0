void launchCo2Notification(uint8_t& a_continueCo2Control, uint8_t& a_previousDate) {
  ...
  if (device::globalDebug)
    Serial.println("About to show co2 cancel timer dialog");
  saveLogMessage(3);
...
void abortCo2Notification(bool& a_startCo2Relay, unsigned long& a_lastTouch, const unsigned long& a_co2RunTime) {
  ...
  while (a_startCo2Relay) {
    ...
    if (tft.touched()) {
      ...
      if (millis() >= a_lastTouch) {
        if (display::touch_x >= startX + 200 && display::touch_x <= startX + 400 && display::touch_y >= startY + 200 && display::touch_y <= startY + 250) { // Cancel
          ...
          if (device::globalDebug)
            Serial.println("Aborted pumping Co2!");
          sensor::co2GasTime = 0;
    ...
    if (millis() > a_co2RunTime) {
      analogWriteChnl(pca9685Channel::co2Solenoid, (device::relayOffState == HIGH ? 4096 : 0));
      if (device::globalDebug)
        Serial.println("Finished pumping Co2");
      sensor::co2GasTime = 0;
      a_startCo2Relay = false;
      clearPage();
      display::refreshPage = true;
...
void launchDrainNotification(uint8_t& a_continueDraining, bool& a_startDraining) {
  ...
  tft.displayOn(true);
  if (device::globalDebug)
    Serial.println("About to show the drain cancel timer dialog");
...
void launchRefillNotification(bool& a_startRefilling, uint8_t& a_continueRefilling) {
  ...
  if (a_startRefilling) {
    ...
    tft.displayOn(true);
    if (device::globalDebug)
      Serial.println("About to show refill cancel timer dialog");
...
void OuterMenuIcons() {
  ...
  if (device::globalDebug) {
    Serial.print("icon time: "); Serial.println(millis() - iconTime);
  }
}
...
void displayDosingNotification(const uint8_t& a_seconds, const float& a_outOfRange, const uint8_t& a_dosingMode, bool& a_refreshDosingNotification) {
  ...
  if (a_refreshDosingNotification) {
    if (device::globalDebug)
      Serial.println("showing dosing alert");
    tft.setFont(&akashi_36px_Regular);
    tft.setFontScale(1);
    //Frame
...
void infoMessage(const char** a_messageArray, const int a_messageIndex) { // message::infoMessageArray, message::infoPos
  ...
  if (display::refreshPage && display::showInfoDialog) {
    if (device::globalDebug)
      Serial.println("Starting infoMessage");
...
void setupMessage(int& a_x, int& a_y, const char* a_message) {
  ...
  //clearPage();
  if (device::globalDebug)
    Serial.println("Starting setupMessage");
  // Work out the dialog height
...
void displaySystemLogs() {
  if (display::refreshPage) {
    ...
    if (device::globalDebug) {   
      Serial.print("message::systemLogPos: "); Serial.println(message::systemLogPos);
      Serial.print("systemLogScrollPos: "); Serial.println(display::systemLogScrollPos);
    }
    ...
    if (device::globalDebug) {
      Serial.print("maxLogsToDsiplay: "); Serial.println(maxLogsToDsiplay);
    }
    for (uint8_t i = display::systemLogScrollPos; i < maxLogsToDsiplay; i++) {
      ...
      if (device::globalDebug) {
        Serial.print("Printing log: "); Serial.print(i); Serial.print(", logType: "); Serial.print(logType); Serial.print(", text: "); Serial.print(logPretext); Serial.print(", time: "); Serial.println(message::timeStrArray[i]);
      }
...
void userSetupGuide() {
  ...
  if (device::globalDebug)
    Serial.println("Starting userSetupGuide");
  display::refreshPage = true;
    ...
    if (tft.touched()) {
      ...
      if (display::touch_x >= startX && display::touch_x <= startX + 150 && display::touch_y >= startY && display::touch_y <= startY + 35) { // continue
        if (millis() - lastTouchTime >= 2000UL) {
          ...
          setupDialogPos++;
          if (device::globalDebug) {
            Serial.print("Continue setup prompt, pos: "); Serial.println(setupDialogPos);  
          }    
          clearPage();     
          display::refreshPage = true;
          lastTouchTime = millis();
