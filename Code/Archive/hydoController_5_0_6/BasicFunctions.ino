void initializeDevice() {
  ...
  tft.setTextColor(RA8875_WHITE, RA8875_BLACK);
  tft.print(120, 370, F("Loading GSL1680..."));
  delay(200);
  if (!i2cScanner(0x40)) {
    tft.fillRect(120, 370, 400, 24, RA8875_BLACK);
    tft.setTextColor(RA8875_RED, RA8875_BLACK);
    tft.print(120, 370, F("Failed to load GSL1680"));
    if (device::globalDebug)
      Serial.println(F("Using resistive touch"));
    tft.touchBegin();
    ...
    if (device::globalDebug)
      Serial.println(F("Using capacitive touch"));
    display::usingCapacitiveTouch = true;
    gsl1680.begin(pin::ra8875_touchWake, pin::ra8875_touchInt); 
  }
  //
  tft.fillRect(120, 370, 400, 24, RA8875_BLACK);
  ...
  tft.fillRect(120, 370, 400, 24, RA8875_BLACK);
  tft.setTextColor(RA8875_WHITE, RA8875_BLACK);
  tft.print(120, 370, F("Loading SCD41..."));
  delay(200);
  if(!SCD4X.begin()) {
    tft.fillRect(120, 370, 400, 24, RA8875_BLACK);
    tft.setTextColor(RA8875_RED, RA8875_BLACK);
    tft.print(120, 370, F("Failed to load SCD41"));
    while(true){};
  }
  ...
  if (user::auxRelayOneMode == 0) {
    ...
    if (device::globalDebug) {
      Serial.print(F("auxRelayOneSwitchedOnOffMin: ")); Serial.println(device::auxRelayOneSwitchedOnOffMin);
      Serial.print(F("auxRelayOneSwitchedOnOffHour: ")); Serial.println(device::auxRelayOneSwitchedOnOffHour);
  ...
  if (user::auxRelayTwoMode == 0) {
    ...
    if (device::globalDebug) {
      Serial.print(F("auxRelayTwoSwitchedOnOffMin: ")); Serial.println(device::auxRelayTwoSwitchedOnOffMin);
      Serial.print(F("auxRelayTwoSwitchedOnOffHour: ")); Serial.println(device::auxRelayTwoSwitchedOnOffHour);
  ...
  if (wifi::wifiEnabled) {
    tft.fillRect(120, 370, 400, 24, RA8875_BLACK);
    tft.setTextColor(RA8875_WHITE, RA8875_BLACK);
    tft.print(120, 370, F("Loading web server..."));
...
void setFanSpeeds(const uint8_t a_fanOneSpeed, const uint8_t a_fanTwoSpeed) {
...
void setRgbLed(const int a_red, const int a_green, const int a_blue) {
  if (!user::disableLED) {
    ...
    analogWriteChnl(device::pca9685Channel::ledRed, a_red);
    analogWriteChnl(device::pca9685Channel::ledBlue, a_green);
    analogWriteChnl(device::pca9685Channel::ledGreen, a_blue);
  }
  else {
    if (device::globalDebug)
      Serial.println(F("RGB LED disabled"));
    analogWriteChnl(device::pca9685Channel::ledRed, 0);
    analogWriteChnl(device::pca9685Channel::ledBlue, 0);
    analogWriteChnl(device::pca9685Channel::ledGreen, 0);
  }
}

void beep() {
  if (!user::disableBeeper) {
    analogWriteChnl(device::pca9685Channel::speaker, 4096);
    delay(60);
    analogWriteChnl(device::pca9685Channel::speaker, 0);
  }
}
