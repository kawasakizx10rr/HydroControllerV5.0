void readSensors() {
  ...
  if (millis() - device::sensorPreviousMillis >= 2000UL && (display::page == 0 || display::page == 1 || display::page == 2 || display::page == 12 || millis() - display::lastTouchMillis > 10000UL)) {
    ...
    // LDR ==============================================================================
    sensor::ldr = analogReadChnl(device::ads1115Channel::ldr);
    ...
    if (swapSenor == 0) {
      if (!device::ecWaitTillNextCall) {
        ...
        float voltage = analogReadChnlToVolts(device::ads1115Channel::tdsSensor);    
    ...
    // PH ==============================================================================
      ...
      if (!device::phWaitTillNextCall) {
        ...
        float voltage = analogReadChnlToVolts(device::ads1115Channel::phSensor);
...
float getWaterHeight() {
  ...
  if (millis() - sensorPreviousMillis >= 1000UL) {
    if (user::heightSensor == user::ETAPE) {
      float resistance = readResistance(device::ads1115Channel::etapeSensor, 560);
...
void setEtapeZeroVolumeResistance() {
  sensor::temp_etapeZeroVolumeResistance = readResistance(device::ads1115Channel::etapeSensor, 560);
  if (device::globalDebug) {
...
void setEtapeMaxVolumeResistance() {
  ...
  sensor::etapeMaxVolumeResistance = readResistance(device::ads1115Channel::etapeSensor, 560);
  if (device::globalDebug) {
