void launchCo2Notification(uint8_t& a_continueCo2Control, uint8_t& a_previousDate) {
  ...
  while (co2DialogTimer != 0 && a_continueCo2Control == device::NOT_SET) {
    ...
    // touch event
    launchCo2NotificationTouch(a_continueCo2Control, a_previousDate);
...
void abortCo2Notification(bool& a_startCo2Relay, unsigned long& a_lastTouch, const unsigned long& a_co2RunTime) {
  ...
  while (a_startCo2Relay) {
    ...
    abortCo2NotificationTouch(a_lastTouch, a_startCo2Relay);
...
void launchDrainNotification(uint8_t& a_continueDraining, bool& a_startDraining) {
  ...
  while (drainingDialogTimer != 0 && a_continueDraining == device::NOT_SET) {
    ...
    // touch event
    launchDrainNotificationTouch(a_continueDraining, a_startDraining);
    static unsigned long previousMillis = millis();
    if (millis() - previousMillis >= 1000UL) {
      rtc.refresh();
      cyclicTimers();
...
void launchRefillNotification(bool& a_startRefilling, uint8_t& a_continueRefilling) {
  ...
  if (a_startRefilling) {
    ...
    while (refillingDialogTimer != 0 && a_continueRefilling == device::NOT_SET) {
      ...
      // touch event
      launchRefillNotificationTouch(a_startRefilling, a_continueRefilling);
      static unsigned long previousMillis = millis();
      if (millis() - previousMillis >= 1000UL) {
        rtc.refresh();
        cyclicTimers();
...
// Draw a radio button, based on bool state
void drawMiniEtapeButton(const int& a_x, const int& a_y, const user::heightSensors& option) {
  tft.setFont(&akashi_36px_Regular);
  tft.setFontScale(1);
  tft.setTextColor(RA8875_BLACK, display::RA8875_DARKGREY);
  ...
  if (option == user::ETAPE)
    tft.print(a_x + 6, a_y - 1, F("Etape"));
...
void displayDosingNotification(const uint8_t& a_seconds, const float& a_outOfRange, const uint8_t& a_dosingMode, bool& a_refreshDosingNotification) {
  ...
  // touch event
  displayDosingNotificationTouch(startX, startY);
}
...
void infoMessage(const char** a_messageArray, const int a_messageIndex) { // message::infoMessageArray, message::infoPos
  ...
  if (display::refreshPage && display::showInfoDialog) {
    ...
    for (unsigned int i = 0; i < strlen_P(a_messageArray[a_messageIndex]); i++) {
      char c = pgm_read_byte_near(a_messageArray[a_messageIndex]+i);
      if (charPos < 32)
        charBuffer[charPos++] = c;
      if (charPos == 32 || c == ' ' || i == strlen_P(a_messageArray[a_messageIndex]) - 1) {
...
void setupMessage(int& a_x, int& a_y, const char* a_message) {
  ...
  // Work out the dialog height
  for (unsigned int i = 0; i < strlen_P(a_message); i++) {
...
// draw a test button at a given x and y
void testButton(const int& a_x, const int& a_y) {
  tft.setFont(&akashi_36px_Regular);
  tft.setFontScale(1);
  tft.setTextColor(RA8875_BLACK, display::RA8875_DARKGREY);
  tft.fillRoundRect(a_x, a_y, 104, 42, 5, display::RA8875_DARKGREY);
  tft.drawRoundRect(a_x - 2, a_y - 2,108, 46, 5, RA8875_BLACK);
  tft.drawRoundRect(a_x - 1, a_y - 1, 106, 44, 5, RA8875_BLACK);
  tft.print(a_x + 10, a_y + 1, F("Test"));
}
...
void drawVerticalSlider(const int& a_x, const int& a_y, const int& a_height, const uint8_t a_percent) {
...
void drawHorizontalSlider(const int& a_x, const int& a_y, const int& a_width, const uint8_t a_percent) {
...
void displaySetBrightness() {
  static int startX, endX;
  static unsigned long previousBrightness;
  if (display::refreshPage) {
    tft.setFont(&akashi_36px_Regular);
    tft.setFontScale(1);
    tft.setTextColor(RA8875_BLACK, user::backgroundColor);
    tft.print(276, 116, "Display brightness");
    drawUpDownButtons(370, 350, 500, 350, RA8875_BLUE);
    exitButton(392, 430);
    infoButton(770, 120);
    tft.setTextColor(RA8875_BLACK, user::backgroundColor);
...
void displaySystemLogs() {
  if (display::refreshPage) {
    ...
    for (uint8_t i = display::systemLogScrollPos; i < maxLogsToDsiplay; i++) {
      ...
      tft.print(105, startX, message::notificationsArray[logType][i]);
...
void userSetupGuide() {
  ...
  if (device::globalDebug)
    Serial.println(F("Starting userSetupGuide"));
  display::refreshPage = true;
    ...
    if (tft.touched()) {
      ...
      if (display::touch_x >= startX && display::touch_x <= startX + 150 && display::touch_y >= startY && display::touch_y <= startY + 35) { // continue
        if (millis() - lastTouchTime >= 2000UL) {
          ...
          setupDialogPos++;
          if (device::globalDebug) {
            Serial.print(F("Continue setup prompt, pos: ")); Serial.println(setupDialogPos);  
          }    
          clearPage();     
          display::refreshPage = true;
          lastTouchTime = millis();
