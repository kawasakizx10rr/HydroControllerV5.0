...
void handleRoot() {
  Serial.print("handleRoot: Executing on core "); Serial.println(xPortGetCoreID());
  ...
  Serial.print("Sending html file, file size: "); Serial.println(sizeof(htmlText));
...
void setValue() {
  Serial.print("setValue: Executing on core "); Serial.println(xPortGetCoreID());
  yield();
  String cmd = server.arg("cmd");
  char val[64] {0};
  server.arg("val").toCharArray(val, 64);
  if (cmd == "" || val[0] == 0) { //Parameters not found
    Serial.println("Web argument not found!");
  }
  else {
    Serial.print("Received cmd: "); Serial.print(cmd); Serial.print(" and val: "); Serial.println(val);
    // ============== PROFILES PAGE  ==============
    // profileOneName
    ...
    // humidityErrorMargin
      ...
      Serial.println("server arg not recognised!");
  }
  server.send(200, "text/plane", "OK");
}

void getPageData() {
  Serial.print("getPagedata: Executing on core "); Serial.println(xPortGetCoreID());
  yield();
  String page = server.arg("page");
  String slide = server.arg("slide");
  Serial.print("Received web request for page "); Serial.print(page); Serial.print(" and slide "); Serial.println(slide);
  ...
  if (strlen(wifi::webData) > 0)
    wifi::webData[strlen(wifi::webData) - 1] = 0;
