void readSensors() {
  ...
  if (millis() - device::sensorPreviousMillis >= 2000UL && (display::page == 0 || display::page == 1 || display::page == 2 || display::page == 12 || millis() - display::lastTouchMillis > 10000UL)) {
    ...
    //Serial.print("water temp in c:"); Serial.println(sensor::waterTemp);
    ...
    if (user::useEtapeSensor) {
      ...
      //Serial.print("emptyWaterTankDepth:"); Serial.println(sensor::emptyWaterTankDepth);
      //Serial.print("Water height in cm:"); Serial.println(sensor::waterLevel);
    }
    ...
    //Serial.print("Air temp in c:"); Serial.println(sensor::airTemp);
    if (sensor::airTemp < 0)
      sensor::airTemp = 0;
    //Serial.print("Humidity:"); Serial.println(sensor::humidity);
    if (sensor::humidity > 99.9)
      sensor::humidity = 99.9;
    // LDR ===============================================================================
    sensor::ldr = analogReadChnl(ads1115Channel::ldr);
    //Serial.print("LDR: "); Serial.println(sensor::ldr);
    // CO2 ===============================================================================
    ...
    if (sensorValue <= 0) {
      if (device::globalDebug) {
        Serial.print("Error reading Co2 sensor buffer: ");
      ...
      if (device::globalDebug)
        Serial.println("Co2 sensor currently preheating...");
      sensor::co2 = 0;
    }
    else {
      sensor::co2 = sensorValue;
      //Serial.print("co2: "); Serial.printn(sensor::co2);
    }
    if (sensor::co2 < 0)
      sensor::co2 = 0;
...
void setTankDepth() {
  ...
  if (distance > 0 && distance < 999) {
    ...
    if (device::globalDebug) {
      Serial.print("EmptyWaterTankDepth = "); Serial.println(sensor::emptyWaterTankDepth, 2);
    }
  }
  else {
...
void generateCo2GasTime() {
  ...
  if (device::globalDebug) {
    Serial.print("Gas time in minues = "); Serial.println(gasTime);
  }
  if (gasTime >= 999)
    gasTime = 999;
  sensor::co2GasTime = gasTime * 60000UL;
  if (device::globalDebug) {
    Serial.print("Co2 gas time in millis = "); Serial.println(sensor::co2GasTime);
  }
}
...
bool phCallbration() {
  ...
  float voltage = analogReadChnlToVolts(ads1115Channel::phSensor);
  if (device::globalDebug) {
    Serial.print("Raw PH voltage = "); Serial.println(voltage, 2);
  }
...
bool tdsCalibration() {
  bool returnVal = false;
  if (device::globalDebug)
    Serial.println("Starting TDS sensor calibration");
  // Turn the PH sensor off
  digitalWrite(pin::phTransistor, LOW);
  // Turn on the TDS sensor
  digitalWrite(pin::tdsTransistor, HIGH);
  ...
  float voltage = analogReadChnlToVolts(ads1115Channel::tdsSensor);    
  if (device::globalDebug) {
    Serial.print("Raw EC voltage = "); Serial.println(voltage, 2);
  }
...
void setEtapeZeroVolumeResistance() {
  ...
  if (device::globalDebug) {
    Serial.print("Etape zero volume resistance = "); Serial.print(sensor::etapeZeroVolumeResistance, 2); Serial.println(" Ohms");
  }
}
...
void setEtapeMaxVolumeResistance() {
  ...
  if (device::globalDebug) {
    Serial.print("Etape max reading resistance = "); Serial.print(sensor::etapeMaxVolumeResistance, 2); Serial.println(" Ohms");
  }
}
